        ...
        <main>
            ...
            <div>
                <mdui-button
                    variant="filled"
                    onClick={() =>
                        createSession(studentId, 'password').then(([data, error]) => {
                            if (error) return void alert({ headline: 'Error', description: error.message })
                            const [token, sid] = data
                            setToken(token)
                            alert({ headline: 'Student session created', description: sid })
                        })
                    }
                >
                    Login as student
                ...
                <mdui-button
                    variant="filled"
                    onClick={() =>
                        createSession(teacherId, 'password').then(([data, error]) => {
                            if (error) return void alert({ headline: 'Error', description: error.message })
                            const [token, sid] = data
                            setToken(token)
                            alert({ headline: 'Teacher session created', description: sid })
                        })
                    }
                >
                    Login as teacher
                ...
                <mdui-button
                    variant="filled"
                    onClick={() =>
                        createSession(adminId, 'password').then(([data, error]) => {
                            if (error) return void alert({ headline: 'Error', description: error.message })
                            const [token, sid] = data
                            setToken(token)
                            alert({ headline: 'Admin session created', description: sid })
                        })
                    }
                >
                    Login as admin
